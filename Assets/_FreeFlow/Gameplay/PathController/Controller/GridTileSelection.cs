...
public class GridTileSelection : MonoBehaviour
{
    ...
    [FormerlySerializedAs("connectionManager")] [SerializeField] private ConnectionController connectionController;
    private bool _isAllowedToInteract;
        ...
        if (hitCollider != null)
        {
            ...
            if (gridTile != null && gridTile.IsNode)
            {
                ...
                if (_paths.ContainsKey(_currentColor))
                {
                    ...
                    connectionController.ClearConnections(_currentColor);
    ...
    private void HandleTileSelection(GridTile gridTile)
    {
        ...
        if (_currentSelection.Count > 1 && _currentSelection[_currentSelection.Count - 2] == gridTile)
        {
            ...
            connectionController.RemoveLastConnection(_currentColor);
        ...
        if (Mathf.Abs(lastTile.Coordinate.x - gridTile.Coordinate.x) +
            Mathf.Abs(lastTile.Coordinate.y - gridTile.Coordinate.y) != 1)
        {
            return;
        }
        ...
        foreach (var kvp in _activeTiles)
        {
            if (kvp.Key != _currentColor && kvp.Value.Contains(gridTile))
            {
                ...
                connectionController.ClearConnections(kvp.Key); // Remove all connections
                break;
            }
        }
        ...
        {
            connectionController.CreateConnection(_currentSelection[_currentSelection.Count - 2], gridTile);
        }
    }
...
//  **Reset Function for Overlapping Paths**
    private void ResetPath(ColorType color)
    {
        ...
        connectionController.RemoveLastConnection(color);
    }
    ...
    private void ValidatePath()
    {
            ...
            _currentSelection[_currentSelection.Count - 1].IsNode)
        {
            ...
            // Highlight all tiles in the completed path
        ...
        else
        {
            ...
            ResetPath(_currentColor);
            connectionController.ClearConnections(_currentColor); // Remove all drawn connections
